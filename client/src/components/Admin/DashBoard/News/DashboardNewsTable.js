...
export default function DashboardNewsTable(props) {
    ...
    if (e.target.id === "Tittle") {
      if (isSortByTitle) {
        ...
        sortByTitle.sort((a, b) => {
          let titleA = a.newTitle.toLowerCase();
          let titleB = b.newTitle.toLowerCase();
  ...
  const choosePage = (e) => {
    ...
    } else if (Number(e.target.id) === 999) {
      setCurrentPage(currentPage + 1);
    } else {
  ...
  return (
    <div className="topfive flex-col" style={{ width: "100%" }}>
      <div className={`headerbox flex-center ${props.colors}`}>
