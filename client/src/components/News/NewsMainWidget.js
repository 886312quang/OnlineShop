...
function NewsMainWidget(props) {
  ...
  const topViews = news.sort((a, b) => b.newView - a.newView).slice(0, 3);
  ...
  return (
    <div className="newsbody-widget">
      <div className="widget-search">
        {props.searchErr && (
          <p styles={{ fontsize: "14px", color: "red" }}>{props.searchErr}</p>
        )}
        <form
          className={
            isSearchFocus ? "widget-form widget_search_click" : "widget-form"
          }
          onSubmit={(e) => {
            e.preventDefault();
...
export default withRouter(NewsMainWidget);
